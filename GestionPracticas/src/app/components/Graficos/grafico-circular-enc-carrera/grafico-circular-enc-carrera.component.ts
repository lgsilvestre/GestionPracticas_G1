...
@Component({
  selector: 'app-grafico-circular-enc-carrera',
  templateUrl: './grafico-circular-enc-carrera.component.html',
  styleUrls: ['./grafico-circular-enc-carrera.component.css']
})
export class GraficoCircularEncCarreraComponent implements OnInit {
...
constructor(private _gestionGraficos: GraficosService) {
  const aprobadas = _gestionGraficos.obtenerInformacionPracticasAprobadasCarrera(this.user.carrera).valueChanges().subscribe(datos => {
    this.datosPracticasAprobadas = datos.length;

    const reprobadas = _gestionGraficos.obtenerInformacionPracticasReprobadasCarrera(this.user.carrera).valueChanges().subscribe(datos => {
      this.datosPracticasReprobadas = datos.length;

      const pendientes = _gestionGraficos.obtenerInformacionPracticasPendientes().valueChanges(this.user.carrera).subscribe(datos => {
